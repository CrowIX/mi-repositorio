...
const totalInfected = persons
  .filter((positive) => positive.infected)
  .map((positive) => positive.infected)
  .reduce((acc, positive) => {
    acc += positive;
    return acc;
  });
...
const totalInfectionsCountries = countries
  .filter((positive) => positive.infected)
  .map((positive) => positive.infected)
  .reduce((acc, positive) => {
    acc += positive;
...
console.log(totalInfectionsCountries);
...
const mostPositivesCountry = countries.reduce((acc, currentCountry) => {
  if (acc.infected < currentCountry.infected) {
    acc = currentCountry;
  }
  return acc;
});
console.log(mostPositivesCountry);
